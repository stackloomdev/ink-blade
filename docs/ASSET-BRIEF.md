...
行走与奔跑可按正常、¼、⅒ 速度播放，底部标尺显示地面相对运动。`src/locomotion.js` 按距离推进步态并固定支撑脚；髋、膝、踝独立联动，回收腿的膝盖始终朝前弯。原图中的可见小腿和手部按轮廓从身体层移除，衣袍仍覆盖在活动肢体前面，避免留下静止的小腿或把衣袍挖出洞。远侧手臂和腿借用同一原画中的近侧部位，属于动作拼接。
