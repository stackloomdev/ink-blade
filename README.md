...
- 主角依据用户设定图重塑：露面长发、黑色披肩、墨染白袍、暗红系带、腰饰与长刀；斗笠保留给普通刀客。
...
[素材清单及生成提示词](docs/ASSET-BRIEF.md) 记录了本次参考图、已实现内容与交接规格。第一批先补主角正、侧、背三视图及刀/刀鞘独立设定。

可编辑底稿：[`hero-reference-blockout.glb`](assets/models/hero-reference-blockout.glb) / [`hero-reference-blockout.blend`](assets/models/hero-reference-blockout.blend)。它们包含分层网格和关节父子结构，尚无蒙皮骨骼、动画片段或最终原画贴图；实战动作与布料仍由代码驱动。源参考图与建模底稿不进入游戏生产构建。
...
- 雨夜表现重构后，Codex 内置浏览器用正常按键输入完成竹林与三阶段 Boss：42 连斩、3 次精准弹反，未修改生命、伤害或墨势。
- 该轮试玩采样 2,311 个显示帧：帧间隔中位数 16.7 ms，95 分位 17.5 ms；这是换用参考图主角前的当前设备样本，不代表所有设备或最终性能。
- 放大检查新主角正面、游戏视角和跑动；六种攻击命中姿势与刀尖数值检查。
- 更换参考图主角后，用正常按键再次通关三阶段 Boss，生命 100、精准弹反 4 次；触发跳跃、空中斩、下劈、挑空、冲刺、绝技和处决，浏览器未记录到异常。
- 主角 GLB 导出后已通过 Blender 导入，保存为包含 148 个网格、22,835 个三角面的 .blend；无蒙皮和动画片段，作为后续精修底稿。
- Safari、手机真机与手柄未验证。
